  ...
  React.useEffect(() => {
    if (session?.user?.guilds) {
      const newTeams = session.user.guilds
        ...
        // 길드를 guildId에 따라서 정렬 (특정 guildId가 첫 번째로 오게)
        .sort((a: Guild, b: Guild) => {
          // 예시로 guildId '123'이 가장 먼저 오게 정렬
          const priorityGuildId = guildId;
          if (a.id === priorityGuildId) return -1; // '123'이 첫 번째로 오도록 설정
          if (b.id === priorityGuildId) return 1;
          return 0; // 그 외의 길드는 순서 그대로
        });
